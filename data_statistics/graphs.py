...
def show_weights_of_svd(stroke_counts, show=False):
    ...
    ----------
    stroke_count : [int]
        A list of all the stroke counts to display. Each stroke count
        is put into its own file and shown in its own figure window.
        ex: [4,6,8] shows pca of characters with 4, 6, and 8 stokes.

    show : bool
        Shows the picture in the display. Default action is to only save
        picture to a file.
    '''
...
def show_data_bar_graph(show=False):
    ...
    ----------
    show : bool
        Shows the graphy in the display. Default action is to only save
        picture to a file.
    '''
