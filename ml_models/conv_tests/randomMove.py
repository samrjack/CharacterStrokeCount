...
n_outputs = enc.n_values_.tolist()[0]


inputs = tf.placeholder(tf.float32, shape=[None, n_inputs])
y_ = tf.placeholder(tf.float32, shape=[None, n_outputs])

a = tf.reshape(inputs, [-1, image_size, image_size, 1])
b = tf.map_fn(lambda image: tf.image.random_brightness(image, max_delta=0.2), a)
c = tf.map_fn(lambda image: tf.image.random_contrast(image, .8, 1.2), b)
d = tf.map_fn(tf.image.random_flip_up_down, c)
x_image = tf.map_fn(tf.image.random_flip_left_right, d)
...
with tf.Session() as sess:
    ...
    for i in range(num_itter):
        ...
        if i % 100 == 0:
            ...
            y_p = prediction.eval(feed_dict={inputs: final_batch[0], keep_prob: 1.0})
            corr = correct.eval({y_: enc.transform(final_batch[1]).toarray()})
            
            f = open(confusion_file, 'w')
            f.write(str(confusion_matrix(y_true=corr, y_pred=y_p)))
            f.close()
        ...
        if i % 35000 == 35000 - 1:
            save_path=saver.save(sess, str(i) + save_file_name)
    # Training Complete
